// <copyright file="OAuthSignatureExtensions.cs" company="Benjamin Howarth &amp; contributors">
...
{
	internal static class OAuthSignatureExtensions
	{
		...
		{
			var nonce = Guid.NewGuid().ToString("N", CultureInfo.DefaultThreadCurrentCulture);
		...
		public static string GenerateSignature(this HttpRequestMessage message, string now, string nonce, string signingKey, Dictionary<string, string> requestParams)
		{
			var requestToBeSigned = string.Concat(
				$"{message.Method.ToString().ToUpperInvariant()}&",
				$"{message.RequestUri.NormalizeUrl().UrlEncode()}&{message.GetParameterString(requestParams, now, nonce)}");
		...
		[SuppressMessage("Encryption", "CA5350", Justification = "Tell Twitter to use a better hash algorithm!")]
